...
export default function Loja() {
    ...
    // Paginação compacta (4 por linha, máx 2 linhas = 8 itens)
    ...
    const renderSkeleton = (key: number) => (
        <div key={key} className="bg-surface-container-low/50 rounded-2xl border border-white/5 p-4 h-[360px] animate-pulse flex flex-col">
            <div className="w-full h-28 bg-white/5 rounded-xl mb-4"></div>
            <div className="h-4 w-1/3 bg-white/5 rounded mb-3"></div>
            ...
            <div className="h-10 w-full bg-white/5 rounded-lg mt-auto"></div>
        </div>
    );
    ...
    const renderProdutoCard = (produto: Produto) => {
        ...
        return (
            <div key={produto.id} className="group relative bg-surface-container-low rounded-[1.5rem] border border-white/5 flex flex-col hover:-translate-y-1.5 hover:border-primary/40 hover:shadow-[0_15px_30px_rgba(140,218,112,0.1)] transition-all duration-500 overflow-hidden h-full">
                ...
                <div className="p-1 flex flex-col h-full relative z-10">
                    <div className="bg-[#121316] rounded-[1.4rem] p-4 flex flex-col h-full relative z-10 shadow-inner shadow-white/5">
                        
                        <div className="w-full h-28 mb-4 rounded-xl overflow-hidden relative bg-surface-container-high border border-white/5 flex items-center justify-center shrink-0">
                            {produto.imagem ? (
                                <img src={produto.imagem} alt={produto.nome} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 opacity-90 group-hover:opacity-100" />
                            ) : (
                                <span className="material-symbols-outlined text-4xl text-white/10 group-hover:scale-110 transition-transform duration-700">deployed_code</span>
                            )}
                            <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-md border border-white/10 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                        ...
                        <div className="mb-4">
                            <h2 className="font-display-lg text-lg text-white font-black mb-1.5 leading-tight tracking-tight line-clamp-2">
                                {produto.nome}
                            </h2>
                            <div className="flex items-baseline gap-1">
                                <span className="text-xs text-primary font-bold">R$</span>
                                <span className="text-2xl text-primary font-black tracking-tighter">
                        ...
                        <button 
                            ...
                            className="w-full py-3 rounded-lg bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest hover:bg-primary hover:text-[#0A1A08] hover:border-primary transition-all text-[11px] mt-auto disabled:opacity-50 flex justify-center items-center h-[42px] group/btn overflow-hidden relative shrink-0 shadow-lg"
                        >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        ...
                        <div className="lg:col-span-9 flex flex-col">
                            {isLoading ? (
                                ...
                                <>
                                    {/* Grade Compacta (4 por linha, máximo 2 linhas) */}
                                    <div key={`${categoriaAtiva}-${servidorAtivo}-${currentPage}`} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6 animate-fade-slide">
                                        {paginatedProducts.map((produto) => renderProdutoCard(produto))}
                                    </div>

                                    {/* Paginação com Setas Cima / Baixo */}
                                    {totalPages > 1 && (
                                        <div className="flex flex-col sm:flex-row items-center justify-between mt-10 pt-6 border-t border-[#374151]">
                                            ...
                                            <div className="flex gap-2">
                                                <button 
                                                    ...
                                                    className="w-10 h-10 rounded-full bg-surface-container border border-[#374151] hover:border-white flex items-center justify-center text-gray-400 hover:text-white disabled:opacity-30 disabled:hover:border-[#374151] disabled:hover:text-gray-400 transition-all shadow-md"
                                                >
